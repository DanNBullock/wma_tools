function [classificationOut] =bsc_segmentSubCortical(wbfg, fsDir,varargin)
%  [classificationOut] =bsc_segmentCorpusCallosum_v2(wbfg, fsDir,varargin)
%
...
for leftright= [1,2]
    ...
    classificationOut=bsc_concatClassificationCriteria(classificationOut,strcat(sideLabel{leftright},'frontoThalamic'),frontoThalamicBool,frontoEndpointBool,frontalBool,~FrontoMotorBool,categoryPrior.index==find(strcmp(categoryPrior.names,'frontal_to_subcortical')),effPrior.index./(length(effPrior.names))>.8);
    classificationOut=bsc_concatClassificationCriteria(classificationOut,strcat(sideLabel{leftright},'temporoThalamic'),temporoThalamicBool,temporalBool,categoryPrior.index==find(strcmp(categoryPrior.names,'temporal_to_subcortical')));
    ...
    classificationOut=bsc_concatClassificationCriteria(classificationOut,strcat(sideLabel{leftright},'parietoThalamic'),parietoThalamicBool,parietalBool,categoryPrior.index==find(strcmp(categoryPrior.names,'parietal_to_subcortical')));
    classificationOut=bsc_concatClassificationCriteria(classificationOut,strcat(sideLabel{leftright},'lentiThalamic'),lentiThalamicBool,lentiBool,categoryPrior.index==find(strcmp(categoryPrior.names,'subcortical_to_subcortical')),effPrior.index./(length(effPrior.names))>.8);%
    motorBoolPrior=or(categoryPrior.index==find(strcmp(categoryPrior.names,'frontal_to_subcortical')),categoryPrior.index==find(strcmp(categoryPrior.names,'parietal_to_subcortical')));
    ...
    classificationOut=bsc_concatClassificationCriteria(classificationOut,strcat(sideLabel{leftright},'spinoThalamic'),spinoThalamicBool,categoryPrior.index==find(strcmp(categoryPrior.names,'spinal_to_subcortical')));
