function roi = dtiRoiFromNiftiObject(nifti,maskValue,outName,outType,binary,save)
% 
% roi = dtiRoiFromNifti([nifti = mrvSelectFile],[maskValue = nonZero],...
%                       [outName=nifti.fname'_maskRoi'],[outType='nifti'],...
%                       [binary=true],[save=true])
%
% 4/22/20 Dan Bullock Note:
% This is the same as dtiRoiFromNifti, except it doesn't bake in the
% loading function.  This is to facilitate faster repeated use of this
% function.
% 
...
% 
% INPUTS:
%       nifti       - Path to nifti image or the object itself.
%       
...
if multiLabel     
    ...
    %DNB question: why wouldn't you justloop over the requested labels?
    for ii=1:length(allLabels)
        if ~ismember(allLabels(ii),maskValue)
            idx = find(nifti.data==allLabels(ii));
            nifti.data(idx) = 0;
        end
    end
end
...
switch lower(outType)
    case {'mat','.mat'}
        ...
        % Use all non-zero points
        if nonZero || multiLabel
            inds = find(nifti.data~=0);
        else
            inds = find(nifti.data==maskValue);
        end
        
        % Convert indexed locations to I J K coords
        [I, J, K] = ind2sub(size(nifti.data),inds);
        
        % Now convert I J K coords to ACPC
        acpcCoords = mrAnatXformCoords(nifti.qto_xyz, [I J K]);
